...
// this code is modified last by Edward on 10/31/2012 Wed.
// this code is compiled last on 11/2/2012 Fri. and succeed 
...
int main(void)
{
    ...
    while(1)
    {
		...
		if (Timer3Flag2==INTERRUPT1S)	//this condition used to send the wireless data or usb data, cause the frequency here is below 10Hz
		{
			...
			wireless_char('\t');
			wireless_int(RealinputGyr);	// send back value from IMU of anglar speed around X
			wireless_char('\t');
